...
class DataPreprocessor:
    ...
    def encode_categorical(self):
        ...
        if 'education' in feature_types['categorical']:
            self.df = pd.get_dummies(self.df, columns=['education'], prefix='edu', drop_first=False)
            logger.info(f"One-hot encoded 'education': {[col for col in self.df.columns if col.startswith('edu')]}")
    ...
    def impute_missing_values(self, X_train, X_test=None):
        ...
        logger.info("Imputing missing values with median strategy...")
    ...
    def split_data(self, test_size=0.2, random_state=42):
        ...
        logger.info(f"Splitting data: train={100-test_size*100:.0f}%, test={test_size*100:.0f}%")
        ...
        logger.info(f"Training set size: {X_train.shape[0]} ({len(y_train[y_train==1])} positive)")
        logger.info(f"Test set size: {X_test.shape[0]} ({len(y_test[y_test==1])} positive)")
...
def preprocess_pipeline(data_path, test_size=0.2, random_state=42, preprocessor_output_path=None):
